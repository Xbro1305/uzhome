    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-5" viewBox="0 0 1440 900" preserveAspectRatio="xMidYMid slice">
          <path d="M0 450 Q360 300 720 450 Q1080 600 1440 450" stroke="#4A6F7A" strokeWidth="1" fill="none"/>
          <path d="M0 350 Q360 200 720 350 Q1080 500 1440 350" stroke="#4A6F7A" strokeWidth="1" fill="none"/>
          <path d="M0 550 Q360 400 720 550 Q1080 700 1440 550" stroke="#4A6F7A" strokeWidth="1" fill="none"/>
        </svg>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 grid lg:grid-cols-2 gap-16 items-center pt-20">
        ...
        <div className="text-center lg:text-left">
          ...
          <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-light leading-tight text-brand-dark mb-6 animate-fade-up">
            Хлопок<br />
            <span className="italic text-brand-primary">природной</span><br />
            чистоты
          </h1>
          <p className="font-body text-base text-brand-muted leading-relaxed max-w-md mx-auto lg:mx-0 mb-10 animate-fade-up" style={{ animationDelay: '0.15s' }}>
            УЗ Хоме — производитель хлопчатобумажной ткани и постельного белья. 
            Мягкость, долговечность и натуральный состав в каждом метре.
          </p>
          ...
          <div className="relative w-[420px] h-[420px]">
            ...
            <div className="absolute inset-12 rounded-full bg-brand-light/30 backdrop-blur-sm flex items-center justify-center">
              <svg width="180" height="180" viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                {/* Stem */}
                <path d="M100 115 Q97 135 90 155" stroke="#4A6F7A" strokeWidth="3" fill="none" strokeLinecap="round"/>
                {/* Leaves */}
                <path d="M90 155 Q75 145 65 148 Q75 160 90 155Z" fill="#4A6F7A" opacity="0.7"/>
                <path d="M90 155 Q85 170 92 178 Q96 165 90 155Z" fill="#4A6F7A" opacity="0.5"/>
                {/* U letter subtle */}
                <path d="M42 40 Q42 65 70 65 Q98 65 98 40" stroke="#4A6F7A" strokeWidth="4" fill="none" strokeLinecap="round" opacity="0.3"/>
                {/* Z letter subtle */}
                <path d="M105 145 L145 145 L105 165 L145 165" stroke="#4A6F7A" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" opacity="0.3"/>
              </svg>
            </div>
            {/* Floating badges */}
            <div className="absolute -top-4 -right-4 bg-white rounded-full px-4 py-2 shadow-lg">
              <span className="font-body text-xs text-brand-primary font-medium">100% Хлопок</span>
            </div>
            <div className="absolute -bottom-4 -left-4 bg-brand-primary rounded-full px-4 py-2 shadow-lg">
              <span className="font-body text-xs text-white font-medium">Сертифицировано</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-bounce opacity-60">
        ...
        <span className="font-body text-[10px] tracking-[0.3em] uppercase text-brand-muted">Листать</span>
      </div>
    </section>
  );
}
